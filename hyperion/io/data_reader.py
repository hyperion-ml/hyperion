...
TransformArg = Optional[Union[TransformList, str]]
ReadIndex = Union[int, List[int], np.ndarray]
...
class DataReader(metaclass=ABCMeta):
    ...
    @staticmethod
    def _squeeze(data: List[np.ndarray], permissive: bool = False) -> np.ndarray:
        ...
        """
        if len(data) == 0:
            return np.array([], dtype=float_cpu())
        ...
        if ref_idx is None:
            if permissive:
                return np.zeros((len(data), 1), dtype=float_cpu())
            raise ValueError("all arrays are empty and permissive=False")

        ndim = data[ref_idx].ndim
        shape = data[ref_idx].shape
        for i in range(len(data)):
            if len(data[i]) == 0:
                if permissive:
                    data[i] = np.zeros((1,) + shape, dtype=float_cpu())
                    continue
                raise ValueError(f"empty array found at index {i}")
    ...
    @staticmethod
    def _combine_ranges(
        read_range: Optional[Tuple[int, int]], row_offset: int, num_rows: int
    ) -> Tuple[int, int]:
        ...
        if read_range[1] > 0:
            assert read_range[1] >= row_offset
            if num_rows == 0:
                num_rows = read_range[1] - row_offset
            else:
    ...
    @staticmethod
    def _apply_range_to_shape(
        shape: Tuple[int, ...], row_offset: int, num_rows: int
    ) -> Tuple[int, ...]:
...
class SequentialDataReader(DataReader):
    ...
    def __init__(
        ...
        """Initialize a sequential reader."""
        super().__init__(file_path, transform, permissive)
        self.lock = multiprocessing.Lock()
        self.part_idx = part_idx
        self.num_parts = num_parts

    def __iter__(self) -> "SequentialDataReader":
    ...
    def next(self) -> Tuple[str, np.ndarray]:
        """__next__ for Python 2"""
        return self.__next__()

    @abstractmethod
    def reset(self) -> None:
        """Returns the file pointer to the beginning of the dataset,
...
class RandomAccessDataReader(DataReader):
    """Abstract base class to read feature matrices by key."""
