...
class TiedVAE_qYqZgY(TiedVAE_qYqZ):
    ...
    def _build_model(self):
        ...
        self.z_param = self.qz_net([x_rep, y])
    ...
    def _get_loss_bernoulli(x, x_dec_param, y_param, z_param,
        ...
        kl_y = K.expand_dims(
            hyp_obj.kl_diag_normal_vs_std_normal(y_param)/seq_length, dim=1)
        
        kl_z = hyp_obj.kl_diag_normal_vs_std_normal(z_param)
        if nb_samples_y > 1:
            r = K.reshape(kl_z, (-1, nb_samples_y, max_seq_length))
    ...
    @staticmethod
    def _get_loss_normal(x, x_dec_param, y_param, z_param, nb_samples, nb_samples_y):
        
        n_seq_samples=K.sum(K.cast(K.any(K.not_equal(x, 0), axis=-1),
                                   K.floatx()), axis=-1)
        ...
        kl_y = K.expand_dims(
            hyp_obj.kl_diag_normal_vs_std_normal(y_param)/n_seq_samples, dim=1)
