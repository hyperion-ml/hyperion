...
def load_input_vectors(hyp_reader, file_path, class_ids, lnorm, max_length):
...
def load_data(iv_file, train_utt2spk_file, val_utt2spk_file, lnorm):
    ...
    [x_train, sw_train] = load_input_vectors(
        hr, train_utt2spk.file_path, train_class_ids, lnorm, max_length)
    [x_val, sw_val] = load_input_vectors(
        hr, val_utt2spk.file_path, val_class_ids, lnorm, max_length)
...
def train_pdda(iv_file, train_utt2spk, val_utt2spk,
    ...
    x_train, sw_train, x_val, sw_val = load_data(
        iv_file, train_utt2spk, val_utt2spk, lnorm)
    ...
    vae.build(max_seq_length = x_train.shape[1])
