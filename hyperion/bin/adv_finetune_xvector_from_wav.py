...
def init_data(
    ...
    """Build dataset, sampler, and dataloader for a data partition.

    Args:
        partition: Data split name (typically ``"train"`` or ``"val"``).
        rank: DDP rank used to gate logging.
        num_gpus: Number of GPUs participating in training.
        **kwargs: Parsed configuration namespace converted to dict.

    Returns:
        A ``torch.utils.data.DataLoader`` for the requested partition.
    """
...
def init_feats(rank: int, **kwargs: Any) -> AF:
    """Initialize waveform-to-feature front-end."""
...
def init_xvector(
    ...
    **kwargs: Any,
    ...
    """Load a pretrained x-vector model and adapt it for fine-tuning."""
...
def init_hard_prototype_mining(
    ...
    """Initialize hard prototype mining state in train/val samplers when enabled."""
    try:
...
def init_attack(
    feat_extractor: nn.Module, model: nn.Module, wav_scale: float, **kwargs: Any
) -> Any:
    """Create the adversarial attack module used during training."""
...
def train_xvec(gpu_id: int, args: Any) -> None:
    """Run adversarial x-vector fine-tuning for one process/GPU."""
...
def make_parser(xvec_class: Any) -> ArgumentParser:
    """Create the parser for one x-vector architecture subcommand."""
    ...
    parser.add_argument(
        "--in-model-file", required=True, help="path to the pretrained x-vector model"
    )
