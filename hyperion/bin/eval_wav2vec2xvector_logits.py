...
def init_device(use_gpu: bool) -> torch.device:
    """Initialise compute device based on ``use_gpu`` flag."""
...
def augment(
    ...
    """Apply optional augmentation to the waveform and record metadata."""
...
def select_random_chunk(
    ...
    """Select a random chunk within ``x`` bounded by ``min``/``max`` seconds."""
...
def eval_xvector_logits(
    ...
    **kwargs: Any,
    ...
    """Compute logits from wav2vec2-xvector models."""
    ...
    if dataset_path is not None and segments_file is not None:
        raise ValueError("--segments-file cannot be used together with --dataset-path")
...
def main() -> None:
    ...
    parser.add_argument(
        "--recordings-file",
        default=None,
        help="Recording specifier used when not reading from a dataset",
    )
    parser.add_argument(
        "--segments-file",
        default=None,
        help="Kaldi segments specifier (only valid when using --recordings-file)",
    )
    parser.add_argument(
        "--vad",
        dest="vad_spec",
    ...
    parser.add_argument(
        "--vad-name",
        default=None,
        help="Name of the dataset VAD entry to use",
    )
    parser.add_argument(
        "--write-speech-dur",
        default=None,
        help="Path to write utt2dur-style speech duration information",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--aug-cfg",
        default=None,
        help="JSON/CFG file describing online augmentation recipes",
    )
    parser.add_argument(
        "--aug-info-path",
        default=None,
        help="Optional CSV file storing augmentation metadata",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model-path", required=True, help="Path to the wav2vec2-xvector model file"
    )
    parser.add_argument(
        "--chunk-length",
        type=float,
