...
def train_embed(data_path, train_list, val_list,
                enc_net_path, pt_net_path, loss,
                init_path, epochs, 
                preproc_file, output_path,
                freeze_enc, freeze_enc_layers, freeze_pt_layers,
                **kwargs):
    ...
    if init_path is None:
        model, cur_epoch = KML.load_checkpoint(output_path, epochs)
        if model is None:
            emb_args = SeqEmbed.filter_args(**kwargs)
            enc_net = load_model_arch(enc_net_path)
            pt_net = load_model_arch(pt_net_path)

            model = SeqEmbed(enc_net, pt_net,
                             loss=loss, **emb_args)
        else:
            kwargs['init_epoch'] = cur_epoch + 1
    else:
    ...
    t1 = time.time()
    if freeze_enc:
        model.freeze_enc_net()
    if freeze_enc_layers is not None:
        model.freeze_enc_layers(freeze_enc_layers)
    if freeze_pt_layers is not None:
        model.freeze_pt_layers(freeze_pt_layers)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--enc-net', dest='enc_net_path', required=True)
    ...
    parser.add_argument('--freeze-enc', dest='freeze_enc',
                        default=False, action='store_true')
    parser.add_argument('--freeze-enc-layers', dest='freeze_enc_layers',
                        default=None, nargs='+')
    parser.add_argument('--freeze-pt-layers', dest='freeze_pt_layers',
                        default=None, nargs='+')
