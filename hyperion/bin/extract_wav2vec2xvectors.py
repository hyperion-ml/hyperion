...
def load_model(model_path: PathLike, device: torch.device) -> TorchModel:
    """Load serialized model and move it to ``device``.
...
def augment(
    ...
    """Apply optional augmentation and register augmentation metadata.
...
def select_random_chunk(
    ...
    """Randomly crop the utterance between ``min`` and ``max`` seconds.
...
def extract_xvectors(
    ...
    model_path: PathLike,
    hf_chunk_length: float,
    xvec_chunk_length: float,
    ...
    **kwargs: Any,
    ...
    """Extract wav2vec2-based x-vectors and write embeddings to disk.
    ...
    Args:
        ...
        hf_chunk_length: Max chunk length (seconds) for HF encoder forward pass.
        xvec_chunk_length: Max chunk length (seconds) for x-vector head pass.
    ...
    if output_spec is None and xvector_path is None:
        raise ValueError(
            "At least one of --xvector-path or --output-spec must be provided"
        )
    ...
    logging.info("opening output stream: %s", writer_spec)
...
def main() -> None:
    """Parse CLI arguments and run wav2vec2 x-vector extraction.
    ...
    parser.add_argument(
        "--dataset-path",
        default=None,
        help="HyperDataset manifest describing recordings/segments/VAD entries",
    )
    parser.add_argument(
        "--recordings-file",
        default=None,
    ...
    parser.add_argument(
        "--vad-file",
        default=None,
        help="Standalone VAD specifier that overrides --vad when present",
    )
    parser.add_argument(
        "--vad-name",
        default=None,
        help="Name of VAD table stored in the dataset manifest",
    )
    parser.add_argument(
        "--write-speech-dur",
        default=None,
    ...
    parser.add_argument(
        "--xvector-path",
        default=None,
        help="Preferred output specifier (ark,h5, etc.) for extracted x-vectors",
    )
    parser.add_argument(
        "--output-spec",
        default=None,
    ...
    if args.output_spec is None and args.xvector_path is None:
        parser.error("Provide --xvector-path or --output-spec")
