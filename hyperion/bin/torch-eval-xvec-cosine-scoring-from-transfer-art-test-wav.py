...
def eval_cosine_scoring(v_file, key_file, enroll_file, test_wav_file, vad_spec,
    ...
    tmodel = MyModel(tfeat_extractor,
                     xvector_tmodel,
                     embed_layer,
                     tcalibrator,
                     threshold=threshold)
    ...
    _, t_x_e = read_data(transfer_v_file, key_file, enroll_file, seg_part_idx,
                         num_seg_parts)
    ...
    for j in range(key.num_tests):
        ...
        trial_time = 0
        num_trials = 0
        model_art = PyTorchClassifier(model=tmodel,
        ...
        logging.info((
            ...
            'rt-factor=%.2f'), key.seg_set[j], t7 - t1, t2 - t1, trial_time,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--transfer-cal-file',
                        default=None,
