...
"""
"""
Trains supervised TCVAE with gmm posteriors.
"""
...
from hyperion.helpers.sequence_class_reader import SequenceClassReader as SR
...
def data_generator(sr, gmm, max_length):
    kk=0
    while 1:
        kk+=1
        # print('dg %d.' % kk)
        ...
        z = np.zeros((x.shape[0], x.shape[1], gmm.num_comp), dtype=float_keras())
        for i in xrange(x.shape[0]):
            z[i] = gmm.compute_z(x[i]).astype(float_keras())
