...
def init_vc_model(
    ...
    Args:
        rank: Process rank in distributed training.
        model_class: VI anonymizer/VC model class to instantiate.
...
def init_audio_feats(rank: int, model_args: Dict[str, Any]) -> AudioFeatsMVN:
    """Initialize audio feature extractor used by loss terms.
...
def train_model(gpu_id: int, args: Any) -> None:
    """Run distributed training for VI anonymizer model.
...
def main() -> None:
    """Parse CLI arguments and launch VI anonymizer training."""
    parser = ArgumentParser(description="Train VI anonymizer model from audio files")
    parser.add_argument(
        "--cfg",
        action=ActionConfigFile,
        help="Path to a configuration file.",
