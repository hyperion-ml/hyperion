        ...
        with DRF.create(input_spec, path_prefix=path_prefix, scp_sep=scp_sep,
            ...
            while not reader.eof():
                ...
                if x.shape[0] == 0:
                    ...
                    with torch.no_grad():
                        n = min(xx.shape[-1], 1000)
                        xx = xx[:,:,:n].to(device)
                        elbo, log_px, kldiv_z, px, qz = model(xx)
                        elbo = elbo.mean().item()
                    ...
                    print(torch.std(xx, dim=(0,2)))
                    print(torch.max(xx, dim=2)[0])
                    print(torch.min(xx, dim=2)[0])
                    logging.info('utt %s elbo=%.2f E[logP(x|z)]=%.2f KL(q(z)||p(z))=%.2f mse=%.2f l1=%.2f' % (
                        key[0], elbo, log_px, kldiv_z, mse, l1))
                    x=xx.cpu().numpy()[0]
                    x_hat=px.mean.cpu().numpy()[0]
                    x_sample=px.rsample().cpu().numpy()[0]
                    vmax = np.max(x)
                    vmin = np.min(x)
                    ...
                    plt.subplot(3,1,2)
                    plt.imshow(x_hat, aspect=2, cmap=cmap, vmax=vmax, vmin=vmin)
                    plt.subplot(3,1,3)
                    plt.imshow(x_sample, aspect=2, cmap=cmap, vmax=vmax, vmin=vmin)

                    plt.savefig('exp/figs/' + key[0] + '.pdf')
