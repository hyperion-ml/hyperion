...
class SPLDA(PLDABase):
    ...
    def __init__(self, x_dim=None, y_dim=None, mu=None, V=None, W=None,
                 num_classes=0, x_ref=None, p_tar=0.05,
        ...
        super().__init__(
            x_dim=x_dim, mu=mu, 
            num_classes=num_classes, x_ref=x_ref, p_tar=p_tar,
            margin_multi=margin_multi, 
            margin_tar=margin_tar, margin_non=margin_non,
            margin_warmup_epochs=margin_warmup_epochs,
            adapt_margin=adapt_margin, adapt_gamma=adapt_gamma,
            lnorm=lnorm,
            var_floor=var_floor, prec_floor=prec_floor)
    ...
    def __str__(self):
        return ('SPLDA(x_dim={}, y_dim={}, num_classes={}, p_tar={}, '
                'margin_multi={}, margin_tar={}, margin_non={}, '
                'margin_warmup_epochs={}, '
                'adapt_margin={}, adapt_gamma={}, '
                'lnorm={}, '
                'var_floor={}, prec_floor={} ').format(
                    self.x_dim, self.y_dim, 
                    self.num_classes, self.p_tar,
                    self.margin_multi, self.margin_tar, self.margin_non, 
                    self.margin_warmup_epochs, 
                    self.adapt_margin, self.adapt_gamma,
                    self.lnorm,          
                    self.var_floor, self.prec_floor)
    ...
    def llr_self(self, x, aux_comps=None):
        ...
        return self._llr_from_Qs(Qtar_11, Qtar_1, Qtar_1, 
                                 Qnon_1, Qnon_1, 
                                 logLtar, logLnon, logLnon)
