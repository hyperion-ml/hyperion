...
class FreeVC(TorchModel):
    ...
    def __init__(
        ...
        assert (
            hf_feats.frame_shift == audio_feats.frame_shift *
            audio_feats.sample_frequency /
            1000), "Mismatch in frame shift between hf_feats and audio_feats"
    ...
    def output_sample_frequency(self) -> int:
        ...
        This is typically the sample frequency of the audio features used in the model.
        """
        return (self.hf_feats.sample_frequency * self.decoder.stride //
    ...
    def max_out_length(self, max_in_length: int) -> int:
        ...
        """
        return self.hf_feats.max_out_length(
            max_in_length) * self.decoder.stride
    ...
    def get_input_context_given_input_length(
            self, input_length: int) -> Tuple[int, int]:
        ...
        """
        max_valid_in_length = (self.hf_feats.max_out_length(input_length) *
                               self.hf_feats.frame_shift)
    ...
    def get_input_matching_output(self, audios: torch.Tensor,
        ...
        """
        start_idx, end_idx = self.get_input_idxs_matching_output(
            audios.shape[-1])
    ...
    def get_target_idxs_matching_output(self, input_length: int) -> List[int]:
        ...
        # max_out_length = self.max_out_length(input_length)
        # max_valid_in_length = max_out_length * self.hf_feats.frame_shift // self.hf_feats.sample_frequency
        max_valid_target_length = (self.hf_feats.max_out_length(input_length) *
                                   self.decoder.stride)
        left_context = (input_length - max_valid_target_length) // 2
        return left_context, left_context + max_valid_target_length

    def get_target_matching_output(self, audios: torch.Tensor,
                                   audio_lengths: torch.Tensor,
        ...
        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Tuple containing the matched audio tensor and lengths.
        """
        start_idx, end_idx = self.get_target_idxs_matching_output(
            max_input_length)
        audios = audios[:, start_idx:end_idx]
        audio_lengths = audio_lengths - start_idx
        audio_lengths = audio_lengths.clamp(min=0, max=audios.shape[-1])
    ...
    def _kldiv_loss(
        ...
        z_mask = seq_lengths_to_mask(z_lengths,
                                     z_flow.size(1),
                                     time_dim=1,
                                     ndim=3,
                                     dtype=z_flow.dtype)
        ...
        with torch.amp.autocast(device_type=z_flow.device.type, enabled=False):
            ...
            kl_div += (0.5 * ((z_flow - prior_z_mean)**2) *
                       torch.exp(-2.0 * prior_z_logs))
    ...
    def forward(
        ...
        """
        feats, feat_lengths = self.forward_hf_feats(source_audios,
                                                    source_audio_lengths)
        if speaker_feats.dim() == 2:
            speaker_feats = speaker_feats.unsqueeze(1)

        if self.l2_norm_speaker:
            speaker_feats = nn.functional.normalize(speaker_feats,
                                                    p=2,
                                                    dim=-1,
                                                    eps=1e-12)
    ...
    def _forward_recons(
        ...
        post_z_flow, flow_logdetJ = self.prior_flow(post_z,
                                                    feat_lengths,
                                                    condition=speaker_feats)
    ...
    def _forward_vc(
        ...
        """
        prior_z, prior_z_mean, prior_z_logs = self.prior_encoder(
            feats, feat_lengths)
        z, _ = self.prior_flow(prior_z,
                               feat_lengths,
                               condition=speaker_feats,
                               reverse=True)
        if slice_start_idxs is not None:
        ...
        gen_audio = self.decoder(z_slice,
                                 x_lengths=z_slice_lengths,
                                 condition=speaker_feats)
        output = FreeVCOutput(
            gen_audio=gen_audio,
            z=z,
    ...
    @staticmethod
    def add_class_args(parser: ArgumentParser,
        ...
        NVPFlow.add_class_args(parser,
                               prefix="prior_flow",
                               skip={"cond_channels", "channels"})
        WaveNetPosteriorEncoder.add_class_args(
            parser,
            prefix="posterior_encoder",
            skip={"in_feats", "cond_channels"})
        HiFiGenerator.add_class_args(parser,
                                     prefix="decoder",
                                     skip={"cond_channels", "in_feats"})
        parser.add_argument(
            "--internal-feats",
            type=int,
            default=192,
