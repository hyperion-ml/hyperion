...
class AudioDataset(Dataset):
    ...
    def __init__(self, audio_path, key_file,
                 class_file = None,
                 time_durs_file = None,
                 min_chunk_length=1,
        ...
        if self.short_seq_exist:
            ...
            for c in range(self.num_classes):
                if class2num_utt[c] > 0:
                    class2max_length[c] = np.max(self.seq_lengths[self.class2utt_idx[c]])
    ...
    def get_random_chunk_length(self):
        ...
        if self.var_chunk_length:
            return torch.rand(size=(1,)).item()*(self.max_chunk_length-self.min_chunk_length)+self.min_chunk_length
