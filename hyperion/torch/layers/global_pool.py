...
class GlobalAvgPool1d(_GlobalPool1d):
    ...
    def forward_slidwin(self, x, win_length, win_shift, snip_edges=False):
        ...
        return self._forward_slidwin_float(x,
                                           win_length,
                                           win_shift,
                                           snip_edges=snip_edges)
    ...
    def _forward_slidwin_int(self, x, win_length, win_shift, snip_edges):
        ...
        c_x, out_shape = self._pre_slidwin(x, win_length, win_shift,
                                           snip_edges)
    ...
    def _forward_slidwin_float(self, x, win_length, win_shift, snip_edges):
        ...
        m_x = torch.zeros((c_x.shape[0], num_frames),
                          dtype=c_x.dtype,
                          device=c_x.device)
        k = 0
...
class GlobalMeanStdPool1d(_GlobalPool1d):
    ...
    def forward(self, x, weights=None):
        if weights is None:
            ...
            s = torch.sqrt(
                torch.mean(delta**2, dim=self.dim,
                           keepdim=False).clamp(min=SQRT_EPS))
...
class GlobalMeanLogVarPool1d(_GlobalPool1d):
    ...
    def forward(self, x, weights=None):
        ...
        wbar = torch.mean(weights, dim=self.dim, keepdim=self.keepdim)
        mu = xbar / wbar
        x2bar = torch.mean(weights * x**2, dim=self.dim,
                           keepdim=self.keepdim) / wbar
...
class LDEPool1d(_GlobalPool1d):
    ...
    def __str__(self):
        s = '{}(in_feats={}, num_comp={}, dist_pow={}, use_bias={}, dim={}, keepdim={})'.format(
            self.__class__.__name__, self.mu.shape[1], self.mu.shape[0],
            self.dist_pow, self.use_bias, self.dim, self.keepdim)
        return s
...
class ScaledDotProdAttV1Pool1d(_GlobalPool1d):
    ...
    def __str__(self):
        s = '{}(in_feats={}, num_heads={}, d_k={}, d_v={}, bin_attn={}, dim={}, keepdim={})'.format(
            self.__class__.__name__, self.in_feats, self.num_heads, self.d_k,
            self.d_v, self.bin_attn, self.dim, self.keepdim)
        return s
    ...
    def forward(self, x, weights=None):
        ...
        if weights is not None:
            mask = weights.view(batch_size, 1, 1,
                                -1).eq(0)  # (batch, 1, 1,time)
            if self.bin_attn:
                scores = scores.masked_fill(mask, 0.0)
                self.attn = scores / (torch.sum(scores, dim=-1, keepdim=True) +
                                      1e-9)
            else:
                min_value = -1e200
                scores = scores.masked_fill(mask, min_value)
                self.attn = torch.softmax(scores, dim=-1).masked_fill(
                    mask, 0.0)  # (batch, head, 1, time)
        else:
            if self.bin_attn:
                self.attn = scores / (torch.sum(scores, dim=-1, keepdim=True) +
                                      1e-9)
                ...
                self.attn = torch.softmax(scores,
                                          dim=-1)  # (batch, head, 1, time)

        x = torch.matmul(self.attn, v)  # (batch, head, 1, d_v)
        if self.keepdim:
            x = x.view(batch_size, 1,
                       self.num_heads * self.d_v)  # (batch, 1, d_model)
        else:
            x = x.view(batch_size,
                       self.num_heads * self.d_v)  # (batch, d_model)
        return x
