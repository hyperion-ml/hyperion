"""
 Copyright 2021 Johns Hopkins University  (Author: Jaejin Cho) - changes in var names from original dino repo to this: student --> model & teacher --> model_teacher
...
class DINOSSLTorchTrainer(object):
    ...
    def __init__(self, model, loss, optim={}, epochs=100, exp_path='./train', 
                 ...
                 cpu_offload=False, niter_per_ep=0, batch_size=0):
        ...
        if ddp: # (JJ: EXP - for now, I will only use self.ddp_type = 'ddp', i.e., DDPType.DDP)
            ...
            else: # (JJ: TODO - Q. why there is NO nn.SyncBatchNorm.convert_sync_batchnorm?)
                if self.rank == 0:
