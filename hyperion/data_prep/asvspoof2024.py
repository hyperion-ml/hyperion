...
class ASVSpoof2024DataPrep(DataPrep):
    ...
    def make_sv_trial_keys(self):
        ...
        file_path = self.output_dir / f"trials_track2.csv"
    ...
    def make_sv_trial_ndxs(self):
        ...
        # drop empty file
        df = df.loc[~df["segmentid"].isin(["D_0000402781"])]
    ...
    def make_spoof_trial_keys(self, segments):
        ...
        tar = np.array(
            [1 if spoof == "bonafide" else 0 for spoof in segments["spoof_det"]],
            dtype=bool,
        )[None, :]
        key = TrialKey(modelid, segmentid, tar, ~tar)
        file_path = self.output_dir / f"trials_track1.csv"
        key.save(file_path)
    ...
    def make_spoof_trial_ndxs(self):
        ...
        ndx = TrialNdx(["bonafide"], df["id"].values)
        file_path = self.output_dir / f"trials_track1.csv"
        ndx.save(file_path)
        # df.to_csv(file_path, index=False)
    ...
    def prepare_enroll(self):
        ...
        rec_ids = [f"ASVSpoof2024-{f}" for f in file_names]
    ...
    def prepare_test(self):
        ...
        if self.subset in ["dev", "eval"]:
            trials = self.make_spoof_trial_keys(segments)
            trials.update(self.make_sv_trial_keys())
        else:
            trials = None
        ...
        dataset.save(self.output_dir)
        logging.info(
            "datasets containts %d segments, %d speakers",
            len(segments),
    ...
    def prepare_progress_eval(self):
        ...
        file_path = self.corpus_dir / f"ASVspoof5.track_1.{self.subset}.trial.txt"
        df_meta = pd.read_csv(
            file_path, sep=" ", header=None, names=["file"], na_values="-"
        )
        df_meta["language"] = "english"
        df_meta.set_index(df_meta.file, drop=False, inplace=True)
        logging.info(f"metadata=\n{df_meta}")
        ...
        trials = self.make_spoof_trial_ndxs()
