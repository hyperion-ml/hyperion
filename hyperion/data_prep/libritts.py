...
class LibriTTSDataPrep(DataPrep):
    """
    Prepares the LibriTTS dataset into structured tables.

    This includes:
    - Speaker, chapter, and book metadata
    - Audio file discovery and duration extraction
    - Segment alignment with transcripts
    - Output of standardized dataset tables

    Attributes:
        corpus_dir (PathLike): Root path to the LibriTTS dataset.
        subset (str): Dataset split (e.g., "train-clean-100").
        output_dir (PathLike): Where the processed files will be saved.
    ...
    def add_class_args(parser) -> None:
        """Adds command-line arguments for LibriTTS options."""
        ...
        parser.add_argument(
            ...
            help="""if we prepare the data for ["train-clean-100", "train-clean-360", "train-other-500", "dev-clean", "dev-other", "test-clean", "test-other"]""",
            required=True,
        )

    def _get_docs_dir(self) -> Path:
        """
        Returns the metadata documentation directory.

        Returns:
            Path: 'docs' subdirectory or fallback to corpus root.
        """
    ...
    def _get_spks_metadata(self) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: Speaker ID and gender.
        """
    ...
    def _get_chapters_metadata(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        Returns:
            Tuple[pd.DataFrame, pd.DataFrame]: Book and chapter metadata.
        """
        docs_dir = self._get_docs_dir()
    ...
    def _get_transcripts(self, rec_dir: Path) -> pd.DataFrame:
        """
        Loads transcripts from transcript TSV files.

        Args:
            rec_dir (Path): Directory to search for .trans.tsv files.

        Returns:
            pd.DataFrame: DataFrame with columns ['id', 'transcript', 'transcript_normalized'].
        """
    ...
    def prepare(self) -> None:
        ...
        logging.info(
            "Peparing LibriTTS %s corpus_dir:%s -> data_dir:%s",
            self.subset,
            self.corpus_dir,
            self.output_dir,
        )
