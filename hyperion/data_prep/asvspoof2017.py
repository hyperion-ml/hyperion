...
class ASVSpoof2017DataPrep(DataPrep):
    """
    Prepares the ASVSpoof2017 dataset into structured tables for training or evaluation.

    Attributes:
        corpus_dir (PathLike): Input directory with original audio and metadata.
        subset (str): Which subset to prepare: 'train', 'dev', or 'eval'.
    ...
    def __init__(
        ...
        """
        Initialize the ASVSpoof2017DataPrep class with input and output settings.
        """
        super().__init__(
    ...
    def add_class_args(parser: ArgumentParser) -> None:
        """
        Add command-line arguments to specify dataset configuration.

        Args:
            parser (ArgumentParser): JSONArgParse-compatible argument parser.
        """
    ...
    def _get_metadata(self):
        """
        Load the metadata protocol file for the given subset.

        Returns:
            pd.DataFrame: Metadata including spoof labels and acoustic conditions.
        """
        ...
        df_meta.loc[:, "spoof_det"] = df_meta["spoof_det"].apply(
            lambda x: "bonafide" if x == "genuine" else x
        )
        df_meta.loc[:, "text"] = df_meta["reddots_sentence"].apply(
            lambda x: sentence_dict[x]
        )
    ...
    def prepare(self):
        """
        Execute the full data preparation pipeline for ASVSpoof2017.
        """
        logging.info(
        ...
        if self.use_kaldi_ids:
            rec_ids = [f'{df_meta.loc[f,"speaker"]}-{f}' for f in file_names]
        else:
            rec_ids = file_names
        ...
        sentence = ClassInfo(
            pd.DataFrame({"id": sentence, "text": [sentence_dict[i] for i in sentence]})
        )

        logging.info("making environment info file")
        environment = np.unique(segments["environment"].dropna())
        environment = ClassInfo(
            pd.DataFrame(
                {"id": environment, "description": [env_dict[i] for i in environment]}
            )
        )

        logging.info("making playback device info file")
        playback_dev = np.unique(segments["playback_device"].dropna())
        playback_dev = ClassInfo(
            pd.DataFrame(
                {
                    "id": playback_dev,
                    "description": [playback_device_dict[i] for i in playback_dev],
                }
            )
        )
        ...
        recording_dev = ClassInfo(
            pd.DataFrame(
                {
                    "id": recording_dev,
                    "description": [recording_device_dict[i] for i in recording_dev],
                }
            )
        )
        ...
        dataset = HypDataset(
            ...
            classes={
                "speaker": speakers,
                "spoof_det": spoof_det,
                "reddots_sentence": sentence,
                "environment": environment,
                "playback_device": playback_dev,
                "recording_device": recording_dev,
                "vocoder": vocoder,
            },
            recordings=recs,
