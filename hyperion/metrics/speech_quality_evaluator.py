...
class SpeechQualityEvaluator:
    ...
    def compute_stats(segments: SegmentSet, stats: Dict[str, Any] = {}):
        ...
        if "utmos" in segments:
            stats["utmos_mean"] = segments["utmos"].mean()
            stats["utmos_std"] = segments["utmos"].std()
            stats["utmos_acc"] = segments["utmos"].sum()
            stats["utmos2_acc"] = (segments["utmos"] ** 2).sum()
    ...
    def accum_stats(stats: List[Union[Dict[str, Any], pd.DataFrame]]):
        ...
        if "utmos_mean" in accum_stats:
            accum_stats["utmos_mean"] = (
                accum_stats["utmos_acc"] / accum_stats["num_segments"]
            )
            accum_stats["utmos_std"] = (
                accum_stats["utmos2_acc"] / accum_stats["num_segments"]
                - accum_stats["utmos_mean"] ** 2
            )
