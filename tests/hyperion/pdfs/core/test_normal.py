...
def test_properties():
    ...
    assert_allclose(
        model.logLambda, 2 * np.sum(np.log(np.diag(la.cholesky(model.Lambda))))
    )
...
def test_initialize():
    ...
    model3 = Normal(mu=model2.mu, Lambda=model2.Lambda, x_dim=model1.x_dim)
    model3.initialize()
...
def test_suff_stats():
    ...
    assert_allclose(
        model1.accum_suff_stats(x, sample_weight=sample_weight)[1], 0.5 * u_x
    )
    assert_allclose(
        model1.accum_suff_stats(x, sample_weight=sample_weight, batch_size=batch_size)[
            1
        ],
        0.5 * u_x,
    )
...
def test_diag_log_prob():
    ...
    assert_allclose(
        model1.log_prob(x, method="nat"), model1_diag.log_prob(x, method="std")
    )
    assert_allclose(
        model1.log_prob(x, method="std"), model1_diag.log_prob(x, method="std")
    )
...
def test_log_prob():
    ...
    assert_allclose(model1.log_prob(x, method="nat"), model1.log_prob(x, method="std"))

    u_x = model1.compute_suff_stats(x)
    assert_allclose(
        model1.log_prob(x, u_x, method="nat"), model1.log_prob(x, method="std")
    )
...
def test_elbo():
    ...
    assert_allclose(model1.elbo(x), np.sum(model1.log_prob(x, method="std")))
    assert_allclose(
        model1.elbo(x, sample_weight=sample_weight),
...
def test_fit():
    ...
    assert_allclose(
        model2.Lambda,
        la.inv(np.dot(x.T, x) / num_samples_train - np.outer(model2.mu, model2.mu)),
    )
