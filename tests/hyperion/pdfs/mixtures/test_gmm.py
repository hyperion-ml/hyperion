...
from hyperion.np.pdfs import GMM, GMMDiagCov
...
mu1 = np.array([[-2, 1.5, -1], [1, 1, 0], [0, -1, 1], [1.5, -1.5, 0.5]])
S1 = np.square(
    np.array([[1, 0.75, 0.5], [0.5, 0.3, 0.1], [0.5, 0.6, 0.7], [0.5, 0.4, 0.3]])
)

S0fc = np.array([[1, 0.3, 0.1], [0.3, 1, -0.25], [0.1, -0.25, 1]])
...
def create_diag_pdf():
    ...
    model_diag = GMMDiagCov(
        num_comp=len(pi1), pi=pi1, mu=mu1, Lambda=1 / S1, x_dim=x_dim
    )
...
def test_diag_initialize():
    ...
    model2 = GMM(
        num_comp=model1.num_comp, pi=model1.pi, eta=model1.eta, x_dim=model1.x_dim
    )
    model2.initialize()
...
def test_suff_stats():
    ...
    N1, u_x1 = model1.accum_suff_stats(x, batch_size=batch_size)
...
def test_suff_stats_segments():
    ...
    print(N.shape)
    print(u_x.shape)
    N1, u_x1 = model1.accum_suff_stats_segments(x, segments, batch_size=batch_size)
...
def test_suff_stats_segments_prob():
    ...
    N1, u_x1 = model1.accum_suff_stats_segments_prob(x, prob, batch_size=batch_size)
...
def test_diag_log_prob():
    ...
    assert_allclose(model1.log_prob(x, mode="nat"), model1_diag.log_prob(x, mode="std"))
    assert_allclose(model1.log_prob(x, mode="std"), model1_diag.log_prob(x, mode="std"))
...
def test_log_prob():
    ...
    assert_allclose(model1.log_prob(x, mode="nat"), model1.log_prob(x, mode="std"))

    u_x = model1.compute_suff_stats(x)
    assert_allclose(model1.log_prob(x, u_x, mode="nat"), model1.log_prob(x, mode="std"))
...
def test_diag_elbo():
    ...
    assert_allclose(
        model1.elbo(x, sample_weight=sample_weight), 0.5 * model1_diag.elbo(x)
    )
...
def test_elbo():
    ...
    ) / num_samples + 0.2 > 0.5 * np.sum(model1.log_prob(x, mode="std"))
