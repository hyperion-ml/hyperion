...
def test_rspecifier():
    ...
    rs1 = RSpecifier(
        RSpecType.ARCHIVE, "file.ark", ArchiveType.ARK, True, True, True, True, True
    )
...
def test_wspecifier():
    ...
    rs1 = WSpecifier(
        WSpecType.ARCHIVE, "file.ark", None, ArchiveType.ARK, False, True, True
    )
    ...
    rs1 = WSpecifier(WSpecType.BOTH, "file.ark", "file.scp", ArchiveType.ARK, False)
    rs2 = WSpecifier.create("ark,t,scp:file.ark,file.scp")
    assert rs1 == rs2

    rs1 = WSpecifier(WSpecType.BOTH, "file.h5", "file.scp", ArchiveType.H5)
    rs2 = WSpecifier.create("h5,scp:file.h5,file.scp")
    assert rs1 == rs2

    rs1 = WSpecifier(WSpecType.BOTH, "file.flac", "file.scp", ArchiveType.AUDIO)
