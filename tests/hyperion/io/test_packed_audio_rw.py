...
from hyperion.io import PackedAudioWriter as AW, SequentialPackedAudioReader as SAR, RandomAccessPackedAudioReader as RAR
...
def gen_signals(num_signals=3):
    ...
    for i in range(num_signals):
        s_i = rng.randn(fs)
        s_i = ((2**15-1)/np.max(np.abs(s_i))*s_i).astype('int16').astype(float_cpu())
        s.append(s_i)
        keys.append('s%d' % i)
