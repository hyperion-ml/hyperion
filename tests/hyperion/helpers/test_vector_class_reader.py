...
def test__filter_by_spc_max_spc():
    ...
    u2c_out = VectorClassReader._filter_by_spc(
        u2c_in, max_spc=4, spc_pruning_mode="last"
    )
    ...
    u2c_out = VectorClassReader._filter_by_spc(
        u2c_in, max_spc=4, spc_pruning_mode="first"
    )
    f = np.ones_like(u2c_in.key, dtype=bool)
    f[1:3] = False
    u2c_gt = Utt2Info.create(u2c_in.key[f], u2c_in.info[f])
    ...
    u2c_out = VectorClassReader._filter_by_spc(
        u2c_in, max_spc=4, spc_pruning_mode="random", rng=rng
    )
...
def test__filter_by_spc_min_max_spc():
    ...
    u2c_out = VectorClassReader._filter_by_spc(
        u2c_in, min_spc=2, max_spc=4, spc_pruning_mode="last"
    )
    f = np.ones_like(u2c_in.key, dtype=bool)
    f[0] = False
    f[5:7] = False
    ...
    u2c_out = VectorClassReader._filter_by_spc(
        u2c_in, min_spc=2, max_spc=4, spc_pruning_mode="first"
    )
    f = np.ones_like(u2c_in.key, dtype=bool)
    f[:3] = False
    u2c_gt = Utt2Info.create(u2c_in.key[f], u2c_in.info[f])
    ...
    rng = np.random.RandomState(1024)
    u2c_out = VectorClassReader._filter_by_spc(
        u2c_in, min_spc=2, max_spc=4, spc_pruning_mode="random", rng=rng
    )
    f = np.ones_like(u2c_in.key, dtype=bool)
    f[0] = False
    f[3] = False
...
def test__split_classes_sequential_nonoverlap():
    ...
    u2c_gt = Utt2Info.create(["1", "2", "3", "4"], ["2", "2", "2", "2"])
    assert u2c_out == u2c_gt
...
def test_vector_class_reader():
    ...
    class_ids_gt = np.array(
        [0, 1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5], dtype="int"
    )
