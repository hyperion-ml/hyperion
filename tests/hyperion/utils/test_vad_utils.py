...
def test_bin_vad_to_timestamps():
    ...
    t_target = np.asarray([
        [0, 25],
        [3*10, 8*10+25],
        [12*10, 13*10+25]], dtype=np.float) - (25.-10.)/2
    t_target[0, 0] = 0
...
def test_timestamps_wrt_vad_to_absolute_timestamps():
    ...
    t_out = timestamps_wrt_vad_to_absolute_timestamps(
        t_in, vad)
...
def test_timestamps_wrt_bin_vad_to_absolute_timestamps():
    ...
    t_out = timestamps_wrt_bin_vad_to_absolute_timestamps(
        t_in, vad, 0.025, 0.010)
...
def test_intersect_segment_timestamps_with_vad():
    ...
    out2speech_target = np.asarray([
        ...
        8, 9, 10, 11, 12, 13, 14, 15, 16], dtype=np.int)
