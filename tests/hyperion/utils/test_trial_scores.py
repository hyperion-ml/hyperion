...
def test_merge():
    ...
    scr2 = TrialScores(
        scr1.model_set[:10], scr1.seg_set, scr1.scores[:10, :], scr1.score_mask[:10, :]
    )
    scr3 = TrialScores(
        scr1.model_set[10:], scr1.seg_set, scr1.scores[10:, :], scr1.score_mask[10:, :]
    )
    ...
    scr2 = TrialScores(
        scr1.model_set, scr1.seg_set[:10], scr1.scores[:, :10], scr1.score_mask[:, :10]
    )
    scr3 = TrialScores(
        scr1.model_set, scr1.seg_set[10:], scr1.scores[:, 10:], scr1.score_mask[:, 10:]
    )
...
def test_load_save():
    ...
    file_h5 = output_dir + "/test_scores.h5"
    scr1.save(file_h5)
    scr2 = TrialScores.load(file_h5)
    assert scr1 == scr2

    file_txt = output_dir + "/test_scores.txt"
