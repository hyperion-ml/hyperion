...
def load_trial_data(
    ...
    return enroll_map, ndx, enroll_segments, enroll_conds, test_segments, x_e, x_t


def load_cohort_data(segments_file, feats_file, source_types, langs):
    segments = SegmentSet.load(segments_file)
    feats_reader = DRF.create(feats_file)
    x = feats_reader.read(segments["id"], squeeze=True)
    segments = get_segment_conditions(segments_file, source_types, langs)
    return segments, x


def apply_preprocessors(x, segments, preprocessors):
...
def eval_backend(
    ...
    enroll_map, ndx, enroll_segments, enroll_conds, test_segments, x_e, x_t = (
        load_trial_data(
            enroll_map_file,
            ndx_file,
            enroll_segments_file,
    ...
    if preproc_file is not None:
        ...
        x_t = apply_preprocessors(x_t, test_segments, preprocessors)
    ...
    if cohort_segments_file is not None:
        ...
        if preproc_file is not None:
            x_coh = apply_preprocessors(cohort_segments, x_coh, preprocessors)
