...
def train_verification_calibration(
    ...
    # gmm_tar = GMMDiagCov(num_comp=1, x_dim=1)
    # gmm_tar.fit(tar[:, :1], epochs=1)
    # gmm_non = GMMDiagCov(num_comp=1, x_dim=1)
    # gmm_non.fit(non[:, :1], epochs=1)
    gmm_tar = GMMDiagCov(num_comp=2, x_dim=1)
    gmm_tar.fit(tar[:, :1], epochs=10)
    gmm_non = GMMDiagCov(num_comp=4, x_dim=1)
    gmm_non.fit(non[:, :1], epochs=10)
    # gmm_tar = GMMDiagCov(num_comp=4, x_dim=5)
    # gmm_tar.fit(tar, epochs=10)
    # gmm_non = GMMDiagCov(num_comp=8, x_dim=5)
    # gmm_non.fit(non, epochs=10)

    mu = np.vstack((gmm_tar.mu, gmm_non.mu))
    Lambda = np.vstack((gmm_tar.Lambda, gmm_non.Lambda))
    pi = np.concatenate((prior * gmm_tar.pi, (1 - prior) * gmm_non.pi))
    gmm = GMMDiagCov(pi=pi, mu=mu, Lambda=Lambda)
    gmm.save(gmm_file)
...
def main():
    ...
    parser.add_argument("--gmm-file", required=True)
