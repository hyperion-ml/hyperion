    ...
    with DWF.create(output_spec, metadata_columns=metadata_columns) as writer:
        ...
        with AR(recordings=recordings_file, **ar_args) as reader:
            ...
            while not reader.eof():
                ...
                logging.info("x-aug-length=%d x-length=%d augmentation=%s", len(x), len(x0), str(aug_info)) 
                t4 = time.time()
                with torch.no_grad():
                    ...
                    if vad_spec is not None:
                        ...
                        vad = torch.tensor(vad[None, None, :], dtype=torch.float).to(
                            device
                        )
                    ...
                    metadata["speech_duration"] = x.shape[1] / model.sample_frequency
                    ...
                    if x.shape[1] == 0:
                        ...
                        y = (
                            model.extract_embed(
                                x, chunk_length=chunk_length, embed_layer=embed_layer,
                            )
                            .cpu()
                            .numpy()[0]
                        )
