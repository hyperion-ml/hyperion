...
def evaluate_attacks(key_file, clean_score_file, attack_score_files, attack_stats_files, 
                     output_path, prior):
    ...
    evaluator = Eval(key_file, clean_score_file, 
                attack_score_files, attack_stats_files, prior)
    ...
    df = evaluator.compute_dcf_eer_vs_stats(
        ...
        'all', higher_better=True, return_df=True)
    ...
    df = evaluator.compute_dcf_eer_vs_stats(
        'snr', [-10, 0, 10, 20, 30, 40, 50, 60], 'tar', 
        higher_better=True, return_df=True)
    ...
    df = evaluator.compute_dcf_eer_vs_stats(
        'snr', [-10, 0, 10, 20, 30, 40, 50, 60], 'non', 
        higher_better=True, return_df=True)
    ...
    for i in range(len(attack_score_files)):
        ...
        evaluator.save_best_attacks(
            file_path, 'snr', 'tar', num_best=10, min_delta=1, attack_idx=i,
            higher_better=True)

        file_path = '%s_best_snr_non_attacks_%d.csv' % (output_path, i)
        evaluator.save_best_attacks(
            file_path, 'snr', 'non', num_best=10, min_delta=1, attack_idx=i,
            higher_better=True)
    ...
    for i in range(len(attack_score_files)):
        ...
        evaluator.save_best_attacks(
            file_path, 'n_linf', 'tar', num_best=10, min_delta=1, attack_idx=i,
            higher_better=False)

        file_path = '%s_best_linf_non_attacks_%d.csv' % (output_path, i)
        evaluator.save_best_attacks(
            file_path, 'n_linf', 'non', num_best=10, min_delta=1, attack_idx=i,
            higher_better=False)
