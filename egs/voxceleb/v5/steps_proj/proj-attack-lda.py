...
def proj_attack_lda(train_v_file, train_list, 
                    test_v_file, test_list, 
                    lda_cols, title, output_path):
    ...
    for col in range(3):
        ...
        for c in range(np.max(class_ids)+1):
            idx = class_ids == c
            plt.scatter(x_lda_trn[idx,0], x_lda_trn[idx,1],
        ...
        for c in range(np.max(class_ids)+1):
            idx = class_ids == c
            plt.scatter(x_lda_test[idx,0], x_lda_test[idx,1],
                        c=color_marker[c][0], marker=color_marker[c][1],
                        label=classes[c])
