...
if [ $stage -le 7 ]; then
    ...
    do
	    ...
	    --random-utt-length true --min-utt-length 4 --max-utt-length 60 --use-bin-vad false \
	    --feat-config $feat_config \
	    --attacks-opts "--attacks-attack-type cw-l2 --attacks-min-confidence 0 --attacks-max-confidence 3 --attacks-min-lr 1e-5 --attacks-max-lr 1e-3 --attacks-min-iter 10 --attacks-max-iter 200 --attacks-norm-time --p-attack 0.25 --random-seed 7000" \
    	    $nnet data/${name}_proc_audio_no_sil \
	    $class2int cw-l2 \
...
if [ $stage -le 8 ]; then
    ...
    do
	    ...
	    --random-utt-length true --min-utt-length 4 --max-utt-length 60 --use-bin-vad false \
	    --feat-config $feat_config \
	    --attacks-opts "--attacks-attack-type cw-linf --attacks-min-confidence 0 --attacks-max-confidence 3 --attacks-min-lr 1e-5 --attacks-max-lr 1e-3 --attacks-min-iter 10 --attacks-max-iter 200 --p-attack 0.25 --random-seed 8000" \
    	    $nnet data/${name}_proc_audio_no_sil \
	    $class2int cw-linf \
...
if [ $stage -le 9 ]; then
    ...
    for name in voxceleb2cat
    do
    	steps_attacks/generate_adv_attacks_xvector_classif.sh  --cmd "$xvec_cmd --mem 12G" --nj 2000 ${xvec_args} \
	    --random-utt-length true --min-utt-length 4 --max-utt-length 60 --use-bin-vad false \
	    --feat-config $feat_config \
	    --attacks-opts "--attacks-attack-type cw-l0 --attacks-min-confidence 0 --attacks-max-confidence 3 --attacks-min-lr 1e-5 --attacks-max-lr 1e-3 --attacks-min-iter 10 --attacks-max-iter 100 --p-attack 0.25 --random-seed 9000" \
    	    $nnet data/${name}_proc_audio_no_sil \
	    $class2int cw-l0 \
