...
class MyModel(nn.Module):
    ...
    def __init__(self, feat_extractor, xvector_model, mvn=None):
...
def init_model(model_path, **kwargs):
    ...
    model = MyModel(feat_extractor, xvector_model, mvn)
    model.eval()
...
def generate_attacks(wav_file, list_file, vad_spec, vad_path_prefix, 
                     class2int_file, model_path, 
                     output_wav_dir, attack_info_file, attack_tag,
                     random_utt_length, min_utt_length, max_utt_length,
                     use_gpu, part_idx, num_parts, random_seed, p_attack, **kwargs):
    ...
    keys, class_names, class_ids = read_utt_list(list_file, class2int_file, part_idx, num_parts)
    ...
    for i in range(len(keys)):
        ...
        if random_utt_length:
            utt_length = torch.randint(
                low=min_utt_length * fs, high=max_utt_length * fs+1, size=(1,)).item()
            if utt_length < len(s):
                first_sample = torch.randint(
                    low=0, high=len(s)-utt_length, size=(1,)).item()
        ...
        if pred[0] != class_id:
            logging.info(
                'utt %s failed benign classification, skipping...' % (key))
            continue
        ...
        if pred[0] != class_id:
            ...
            logging.info(
                'utt %s attack failed, skipping...' % (key))
        ...
        logging.info((
            ...
            'rt-factor=%.4f') % (
                key, t6-t1, t2-t1, t3-t2, t4-t3, t5-t4,
                s.shape[1]/fs/(t6-t1)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--part-idx', default=1, type=int,
                        help=('part index'))
    parser.add_argument('--num-parts', default=1, type=int,
                        help=('number of parts in which we divide the list '
    ...
    parser.add_argument('--min-utt-length', type=int, default=5, 
                        help=('minimum utterance length (in secs) when using random utt length'))
    parser.add_argument('--max-utt-length', type=int, default=120, 
                        help=('maximum utterance length (in secs) when using random utt length'))

    parser.add_argument('--p-attack', type=float, default=1, 
