...
nnet_dir=${nnet_dir}_Noclassbalancebatching_correct_fclikeimage_revertifNOTwork
...
if [ $stage -le 1 ]; then
    ...
    if [[ ${dinossl} == true ]]; then
        ...
        for arg in dinossl_fclikeimage dinossl_keep_classif_net dinossl_nlayers dinossl_out_dim dinossl_use_bn_in_head dinossl_norm_last_layer \
    ...
    $cuda_cmd_30G --gpu $ngpu $nnet_dir/log/train.log \
