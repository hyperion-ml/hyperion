...
class_file=data/exp_attack_type_v1/class2int
...
list_dir=data/exp_verif_attack_type_v1

if [ $stage -le 1 ];then
    # concatenate infos of all attacks types
    mkdir -p $attack_dir/pool_voxceleb1_test_v1
    for attack in fgsm iter-fgsm cw-l2 cw-linf cw-l0 pgd-linf pgd-l1 pgd-l2
    do
	...
	do
	    cat $attack_dir/$attack/$name/info/info.yml
                               ...
                               { print $0 }'
    done | awk '!/\{\}/' > $attack_dir/pool_voxceleb1_test_v1/info.yml

    local/make_verif_test_lists_exp_attack_type_v1.py \
