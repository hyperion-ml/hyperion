...
from hyperion.metrics.confusion_matrix import compute_xlabel_confusion_matrix, print_confusion_matrix
...
def eval_classif_perf(score_file, key_file, class_file, output_path=None, **kwargs):
    ...
    C = compute_xlabel_confusion_matrix(y_true, y_pred, labels_train=labels_train, normalize=False)

    logging.info('Unnormalized Confusion Matrix:')
    print_confusion_matrix(C, labels_true=classes_test, labels_pred=classes)

    Cn = compute_xlabel_confusion_matrix(y_true, y_pred, labels_train=labels_train, normalize=True)
    logging.info('Normalized Confusion Matrix:')
    print_confusion_matrix(Cn*100, labels_true=classes_test, labels_pred=classes, fmt='.1f')
