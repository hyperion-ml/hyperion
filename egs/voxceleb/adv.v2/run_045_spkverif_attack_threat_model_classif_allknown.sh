...
class_file=data/$threat_model_split_tag/class_file
list_dir=data/$spkverif_threat_model_split_tag
list_attack_type_dir=data/$spkverif_attack_type_split_tag

conda activate $HYP_ENV

if [ $stage -le 1 ];then
    echo "make data directory for speaker verification attack classification by threat model"
    local/make_spkverif_test_lists_exp_attack_threat_model_v1.py \
