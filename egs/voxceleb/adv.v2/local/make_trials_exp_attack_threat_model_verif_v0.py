...
def make_lists(input_dir, seen_attacks, benign_wav_file, max_trials, num_enroll_sides, output_dir):
    ...
    if num_enroll_sides > 1:
        ...
        enr_e2tm = Utt2Info.create(enroll_models_uniq, test_u2tm.info[idx])
    else:
        enr_e2c = enr_u2c
        enr_e2tm = enr_u2tm
        enr_u2e = Utt2Info.create(enr_u2c.key, enr_u2c.key)

    trials_all = TrialKey(enr_e2tm.key, test_u2c.key)
    trials_seen = TrialKey(enr_e2tm.key, test_u2c.key)
    trials_unseen = TrialKey(enr_e2tm.key, test_u2c.key)
