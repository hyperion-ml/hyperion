...
if [ $stage -le 0 ];then
  ...
  score_plda_dir=$score_dir/cosine_benign
  echo "Eval Voxceleb 1 with Cosine scoring benign"
  steps_xvec/eval_cosine_scoring_from_test_wav.sh --cmd "$eval_cmd" $eval_args --nj 20 \
...
if [ $stage -le 8 ];then
	...
	do
	    for it in 100
	    do
		score_plda_dir=$score_dir/cosine_cwl0_conf${confidence}_lr${lr}_noabort_it$it
...
if [ $stage -le 9 ];then
	...
	do
		...
		steps_adv/eval_cosine_scoring_from_adv_test_wav.sh --cmd "$eval_cmd" $eval_args --nj 160 \
		    --feat-config $feat_config  \
...
if [ $stage -le 10 ];then
	...
	do
		...
		score_plda_dir=$score_dir/cosine_pgd${it}linf_e${eps}_a${alpha}_nri$nri
...
if [ $stage -le 11 ];then
	...
	do
		...
		do
		    alpha=$(echo $eps | awk '{ print 2*$0/'$it'.}')
		    score_plda_dir=$score_dir/cosine_pgd${it}l${norm}_e${eps}_a${alpha}_nri$nri
