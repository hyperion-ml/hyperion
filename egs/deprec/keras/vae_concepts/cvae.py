...
def vae(file_path):
    ...
    h1 = Dense(h_dim, activation='relu', kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(xr)
    h2 = Dense(h_dim, activation='relu', kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(h1)
    z_mean = Dense(z_dim, kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(h2)
    z_logvar = Dense(z_dim, kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(h2)
    
    encoder=Model([x, r], [z_mean, z_logvar])
    
    # define decoder architecture
    z=Input(shape=(z_dim,))
    zr = Concatenate(axis=-1)([z, r])
    h1_dec = Dense(h_dim, activation='relu', kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(zr)
    h2_dec = Dense(h_dim, activation='relu', kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(h1_dec)
    x_dec_mean = Dense(x_dim, kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(h2_dec)
    x_dec_logvar = Dense(x_dim, kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(h2_dec)
