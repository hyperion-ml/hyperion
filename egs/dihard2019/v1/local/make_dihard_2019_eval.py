...
# This script is called by local/make_dihard_2019_eval.sh, and it creates the
# necessary files for DIHARD 2019 evaluation directory.

import sys, os

def prepare_dihard2019_eval(src_dir, data_dir):
    data_dir = os.path.join(data_dir, 'data/single_channel')
        ...
        for file in files:
            ...
            if filename.endswith(".lab"):
                ...
                for line in lines:
                    ...
                    segment_id += 1
                wav_str = "{} sox -t flac {}/flac/{}.flac -t wav -r 16k "\
                        "-b 16 - channels 1 |\n".format(utt, src_dir, utt)
                wavscp_fi.write(wav_str)
                with open("{}/rttm/{}.rttm".format(src_dir, utt), 'r') as fh:
                    rttm_str = fh.read()
...
def main():
    ...
    prepare_dihard2019_eval(src_dir, data_dir)
    return 0
