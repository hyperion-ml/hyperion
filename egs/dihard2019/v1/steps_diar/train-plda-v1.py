...
def train_plda(v_file, train_list,
               lda_dim, plda_type, y_dim, z_dim, epochs, ml_md, md_epochs,
               output_path, inter_session, subsampling, **kwargs):

    keys, class_ids = load_train_list(train_list, inter_session)
    logging.info('reading {} utts with {} classes'.format(
        len(keys), np.max(class_ids)+1))
    x, class_ids = load_vectors(v_file, keys, class_ids, subsampling)
    logging.info('read x={} with num_classes={}'.format(
        x.shape, np.max(class_ids)+1))
    ...
    pd.DataFrame({ 'epochs': np.arange(1, epochs+1, dtype=np.int),
                   'elbo': elbo,
                   'elbo_per_sample': elbo_norm}).to_csv(
                       output_path + '/elbo.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--v-file', required=True, 
                        help='embedding read specifier')
    parser.add_argument('--train-list', required=True, 
                        help='train list utterance spkid')
    ...
    parser.add_argument('--lda-dim', type=int,
                        default=None)
    parser.add_argument('--inter-session', default=False, action='store_true',
                        help=('if True, model inter-session variability, '
                              'if False, model intra-session variability'))
    parser.add_argument('--subsampling', default=1, type=int,
                        help=('subsamples the embeddings to reduce memory and '
                        'computing cost'))

    parser.add_argument('-v', '--verbose', dest='verbose', 
                        default=1, choices=[0, 1, 2, 3], type=int)
