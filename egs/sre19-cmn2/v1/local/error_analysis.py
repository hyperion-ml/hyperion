...
def score_dcf(key_file, score_file, output_path):
    ...
    tar_enr_scr = np.sum(scr.scores * key.tar, axis=1)/(np.sum(key.tar, axis=1)+1e-5)
    tar_tst_scr = np.sum(scr.scores * key.tar, axis=0)/(np.sum(key.tar, axis=0)+1e-5)
    non_enr_scr = np.sum(scr.scores * key.non, axis=1)/(np.sum(key.non, axis=1)+1e-5)
    non_tst_scr = np.sum(scr.scores * key.non, axis=0)/(np.sum(key.non, axis=0)+1e-5)
