...
def rttm2vad_file(file_id, rttm, num_frames, fvad, fu2o, min_dur):
    ...
    if len(spk_ids) == 1:
        ...
        tend = min(
            np.round((rttm.tbeg + rttm.tdur) / frame_shift).astype("int"), num_frames
        )
        vad[tbeg : tend + 1] = 1
    ...
    for i in range(num_spks):
        if total_dur[i] >= min_dur or do_all:
            ...
            tend = np.round(
                np.array(rttm.tbeg.loc[idx] + rttm.tdur.loc[idx]) / frame_shift
            ).astype("int")
...
def rttm2vad(rttm_file, num_frames_file, vad_file, utt2orig, min_dur):
    ...
    df_num_frames = pd.read_csv(
        num_frames_file, sep="\s+", header=None, names=["file_id", "num_frames"]
    )
