...
def read_rttm(rttm_file, uem_file=None, sep=' '):
    ...
    if uem_file is not None:
        ...
        if np.sum(index_fix) > 0:
            ...
            rttm_uem.loc[index_fix, 'tdur'] = rttm_uem[index_fix].file_tend - rttm_uem[index_fix].tbeg
        ...
        index_fix=(rttm_uem['tbeg'] < rttm_uem['file_tbeg']) & (rttm_uem['tbeg'] + rttm_uem['tdur']> rttm_uem['file_tbeg'])
        if np.sum(index_fix) > 0:
            print('fixing %d segments that start before file tbeg' % (np.sum(index_fix)))
            #print(rttm_uem[index_fix])
            rttm_uem.loc[index_fix, 'tdur'] = rttm_uem[index_fix].tbeg + rttm_uem[index_fix].tdur - rttm_uem[index_fix].file_tbeg
...
def write_wav(df_wav, df_segments, output_path):
    ...
    with open(output_path + '/wav.scp', 'w') as f:
        for segment_id, file_id, tbeg, tend in zip(
                df_segments['segment_id'], df_segments['filename'],
                df_segments['beginning_time'], df_segments['end_time']):
            file_path = df_wav.loc[file_id,'file_path']
            f.write('%s sox -t flac %s -t wav - trim %.3f =%.3f | \n' % (
...
def make_dihard_train(rttm_file, uem_file, wav_path, output_path, min_dur, max_dur, data_prefix):
