...
if [ "$print_header" == "true" ];then
    ...
    printf "\n"
    nc=$(echo $n_dbs1*$n_c1 + $n_dbs2*$n_c2 + $n_dbs3*$n_c3 + $n_dbs4*$n_c4| bc)
    for((c=0;c<$nc;c++))
    do
	printf "$measures"
    done
...
do
    ...
    do
	res_file=$score_dir/sitw_${db}_${c}_results
...
for db in eval
do
    res_file=$score_dir/sre18_${db}_vast_results
    awk  '{ printf "%.2f,%.3f,%.3f,", $2,$4,$6}' $res_file
done

for db in dev eval
do
    res_file=$score_dir/sre19_av_a_${db}_results
    awk  '{ printf "%.2f,%.3f,%.3f,", $2,$4,$6}' $res_file
done
