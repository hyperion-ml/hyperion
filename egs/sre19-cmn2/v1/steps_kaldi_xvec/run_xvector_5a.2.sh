...
# This script trains Factorized TDNN with skip connections used in NIST SRE18 JHU-MIT submission.
...
data=data/train
nnet_dir=exp/xvector_nnet_3a/
egs_dir=exp/xvector_nnet_3a/egs
...
if [ $stage -le 7 ]; then
  ...
  relu-batchnorm-layer name=tdnn1 input=Append(-2,-1,0,1,2) dim=2048
  linear-component name=tdnn2l dim=512 $linear_opts input=Append(-2,0)
  relu-batchnorm-layer name=tdnn2 $opts input=Append(0,2) dim=2048
  linear-component name=tdnn3l dim=512 $linear_opts input=Append(-3,0)
  relu-batchnorm-layer name=tdnn3 $opts input=Append(0,3) dim=2048 
  linear-component name=tdnn4l dim=512 $linear_opts 
  relu-batchnorm-layer name=tdnn4 $opts input=Append(0,tdnn2l) dim=2048
  linear-component name=tdnn5l dim=512 $linear_opts input=Append(-3,0)
  relu-batchnorm-layer name=tdnn5 $opts input=Append(0,3) dim=2048
  linear-component name=tdnn6l dim=512 $linear_opts
  relu-batchnorm-layer name=tdnn6 $opts input=Append(0,tdnn4l) dim=2048
  linear-component name=tdnn7l dim=512 $linear_opts input=Append(-3,0)
  relu-batchnorm-layer name=tdnn7 $opts input=Append(0,3) dim=2048
  linear-component name=tdnn8l dim=512 $linear_opts
  relu-batchnorm-layer name=tdnn8 $opts input=Append(0,tdnn6l) dim=2048
  linear-component name=tdnn9l dim=512 $linear_opts input=Append(-3,0)
  relu-batchnorm-layer name=tdnn9 $opts input=Append(0,3) dim=2048
  linear-component name=tdnn10l dim=512 $linear_opts
  relu-batchnorm-layer name=tdnn10 $opts input=Append(0,tdnn8l) dim=2048
  linear-component name=tdnn11l dim=512 $linear_opts input=Append(-3,0)
  relu-batchnorm-layer name=tdnn11 $opts input=Append(0,3) dim=2048
  linear-component name=tdnn12l dim=512 $linear_opts
  relu-batchnorm-layer name=tdnn12 $opts input=Append(0,tdnn10l) dim=2048
...
EOF
  ...
  echo "output-node name=output input=tdnn13.affine" > $nnet_dir/extract.config
  echo "$max_chunk_size" > $nnet_dir/max_chunk_size
  echo "$min_chunk_size" > $nnet_dir/min_chunk_size
fi

dropout_schedule='0,0@0.10,0.25@0.50,0.50@0.75,0.25'
srand=123
if [ $stage -le 8 ]; then
