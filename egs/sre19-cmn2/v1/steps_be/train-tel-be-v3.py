...
def train_be(iv_file, train_list,
             ...
             unlab_adapt_iv_file, unlab_adapt_list,
             lda_dim,
             plda_type, y_dim, z_dim,
             epochs, ml_md, md_epochs,
             w_mu1, w_B1, w_W1,
             w_mu2, w_B2, w_W2, num_spks_unlab, do_ahc,
             w_coral_mu, w_coral_T,
             output_path, **kwargs):
    ...
    plda = F.create_plda(plda_type, y_dim=y_dim, z_dim=z_dim,
                         name='plda')
    elbo = plda.fit(x_lab_ln, class_ids_lab_tot, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--unlab-adapt-iv-file', dest='unlab_adapt_iv_file', required=True)
    ...
    parser.add_argument('--lda-dim', dest='lda_dim', type=int,
                        default=150)
    parser.add_argument('--w-coral-mu', dest='w_coral_mu', type=float,
                        default=0.5)
    parser.add_argument('--w-coral-t', dest='w_coral_T', type=float,
                        default=0.75)
    ...
    parser.add_argument('--w-w2', dest='w_W2', type=float,
                        default=1)
    parser.add_argument('--num-spks-unlab', dest='num_spks_unlab', type=int,
                        default=1000)
