...
def eval_plda(iv_file, ndx_file, enroll_file, test_file,
              preproc_file,
              model_file, score_file, plda_type, pool_method,
              model_part_idx, num_model_parts,
    ...
    f, idx = ismember(ndx.model_set, enroll)
    assert np.all(f)
    scores = scores[idx] 
    s = TrialScores(ndx.model_set, ndx.seg_set,
                    scores, score_mask=ndx.trial_mask)
