...
from hyperion.keras.vae import TiedCVAE_qYqZgY as TVAE
...
def vae(file_path):
    ...
    file_path2 = file_path + '/tied_cvae_qyqzgy'
    ...
    r = Input(shape=(N_i, K,))
    xr = Concatenate(axis=-1)([x, r])
    h1_y = TimeDistributed(Dense(h_dim, activation='relu',
                                 kernel_initializer=my_init, kernel_regularizer=l2(l2_reg)))(xr)
    h2_y = TimeDistributed(Dense(int(h_dim/2), activation='relu',
                                 kernel_initializer=my_init, kernel_regularizer=l2(l2_reg)))(h1_y)
    h2_y_pool = GlobalAveragePooling1D()(h2_y)
    y_mean = Dense(y_dim, kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(h2_y_pool)
    y_logvar = Dense(y_dim, kernel_initializer=my_init, kernel_regularizer=l2(l2_reg))(h2_y_pool)
    qy=Model([x, r],[y_mean, y_logvar])
    
    # define q(z|x,y)
    y1=Input(shape=(N_i, y_dim,))
    xyr = Concatenate(axis=-1)([x, y1, r])
    h1_z = TimeDistributed(Dense(h_dim, activation='relu',
                                 kernel_initializer=my_init, kernel_regularizer=l2(l2_reg)))(xyr)
    h2_z = TimeDistributed(Dense(int(h_dim/2), activation='relu',
                                 kernel_initializer=my_init, kernel_regularizer=l2(l2_reg)))(h1_z)
    ...
    z_logvar = TimeDistributed(Dense(z_dim, kernel_initializer=my_init,
                                     kernel_regularizer=l2(l2_reg)))(h2_z)
    qz=Model([x, y1, r],[z_mean, z_logvar])
    
    # define decoder architecture
    y=Input(shape=(N_i, y_dim,))
    z=Input(shape=(N_i, z_dim,))
    yzr = Concatenate(axis=-1)([z, y, r])
    
    h1_dec = TimeDistributed(Dense(h_dim, activation='relu',
                                kernel_initializer=my_init, kernel_regularizer=l2(l2_reg)))(yzr)
    ...
    save_hist(file_path2 + '_hist.h5', h.history, 'TiedCVAE Q(y)Q(Z|y)')
