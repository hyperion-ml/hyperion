...
if [ $stage -le 2 ];then
    ...
    do
	cond_i=${sitw_conds[$i]}
	steps_be/eval_vid_be_diar_v2.sh --cmd "$train_cmd" \
	    --plda_type $plda_type \
...
if [ $stage -le 3 ]; then
    ...
    steps_be/eval_vid_be_diar_v2.sh --cmd "$train_cmd" --plda_type $plda_type \
...
if [ $stage -le 9 ]; then
    ...
    steps_be/eval_vid_be_diar_snorm_v2.sh --cmd "$train_cmd" --plda_type $plda_type --ncoh $ncoh_vast \
    	data/sre19_av_a_dev_test/trials \
    	data/sre19_av_a_dev_enroll/utt2spk \
    	$xvector_dir/sre19_av_a_dev_enroll/xvector.scp \
    	$xvector_dir/sre19_av_a_dev_test_${diar_name}/xvector.scp \
...
if [ $stage -le 10 ]; then
    ...
    steps_be/eval_vid_be_diar_snorm_v2.sh --cmd "$train_cmd" --plda_type $plda_type --ncoh $ncoh_vid \
    	data/janus_dev_test_core/trials \
    	data/janus_dev_enroll/utt2spk \
    	$xvector_dir/janus_dev_enroll/xvector.scp \
    	$xvector_dir/janus_dev_test_core_${diar_name}/xvector.scp \
