...
def apply_ahc(v_file, input_list, output_list,
              preproc_file, model_file, plda_type,
              cal_file, score_hist_file, threshold,
              pool_method,  coh_nbest, class_prefix, **kwargs):
    ...
    if score_hist_file is not None:
        plt.hist(scores.ravel(), 1000, histtype='step', density=True, color='b',
    ...
    if class_prefix is not None:
        class_ids_ahc = np.asarray(['%s-%06d' % (class_prefix, i) for i in class_ids_ahc])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pool-method', type=str.lower,
                         default='vavg-lnorm',
