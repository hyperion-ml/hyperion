...
def train_fusion(score_files, system_names, key_file, model_file, prior, prior_eval, 
                 lambda_reg, solver, max_systems, verbose):
    num_systems=len(score_files)
    assert num_systems == len(system_names), 'len(score_files)(%d) != len(system_names)(%d)' % (
    ...
    gf = GF(system_names=system_names, prior=prior, prior_eval=prior_eval, 
            lambda_reg=lambda_reg, solver=solver, max_systems=max_systems,
            verbose=verbose)
    gf.fit(x, y)
    ...
    for i in range(len(tar_fus)):
        ...
        for j in range(len(gf.prior_eval)):
            ...
            info_str='%s (p=%.3f) min_dcf: %.3f act_dcf: %.3f p_miss: %.2f p_fa: %.2f n_miss: %.1f n_fa: %.1f' % (
                info_str, gf.prior_eval[j], min_dcf[j], act_dcf[j], p_miss[j]*100, p_fa[j]*100, n_miss, n_fa)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--score-files', dest='score_files', nargs='+', required=True)
    parser.add_argument('--system-names', dest='system_names', nargs='+', required=True)
    ...
    parser.add_argument('--prior', dest='prior', type=float,
                        default=0.01)
    parser.add_argument('--prior-eval', dest='prior_eval', type=float, nargs='+',
                        default=None)
    ...
    parser.add_argument('--solver', dest='solver', 
                        ...
                        default='liblinear')
    parser.add_argument('--max-systems', dest='max_systems', type=int,
                        default=10)
