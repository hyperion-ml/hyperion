...
def train_calibration_cond(cond, scr, key, model_file, prior, lambda_reg, verbose):
    ...
    n_miss = p_miss*ntar
    n_fa = p_fa*nnon
    logging.info('cond %s min_dcf: %.3f p_miss: %.2f p_fa: %.2f n_miss: %.1f n_fa: %.1f' % 
    ...
    n_miss = p_miss*ntar
    n_fa = p_fa*nnon
    logging.info('cond %s act_dcf: %.3f p_miss: %.2f p_fa: %.2f n_miss: %.1f n_fa: %.1f' % 
                 (cond, act_dcf, p_miss*100, p_fa*100, n_miss, n_fa))

    return tar_cal, non_cal


def train_calibration(score_file, key_file, model_file, cond_file, prior, lambda_reg, verbose):
    ...
    conds, cond_ids = np.unique(enr2cond.filter(key.model_set).info, return_inverse=True)
    num_conds = len(conds)
    ntar = len(tar)
    nnon = len(non)
