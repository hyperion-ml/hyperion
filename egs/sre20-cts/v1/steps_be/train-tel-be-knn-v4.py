...
def train_dplda(x, class_ids, plda_init, output_path):
    ...
    sampler = ClassWeightedEmbedSampler(dataset, batch_size=batch_size, num_egs_per_class=4)
    train_loader = torch.utils.data.DataLoader(
        dataset, batch_sampler = sampler)

    sampler_val = ClassWeightedEmbedSampler(dataset, batch_size=batch_size, num_egs_per_class=4)
    val_loader = torch.utils.data.DataLoader(
        dataset, batch_sampler = sampler_val)
    ...
    model = TSPLDA(mu=plda_init.mu, V=plda_init.V, W=plda_init.W, 
                   num_classes=np.max(class_ids)+1, x_ref=x_ref, lnorm=True, 
                   margin_multi=0, margin_tar=30, margin_non=30, adapt_margin=False, margin_warmup_epochs=25) #it was 50

    optimizer = OF.create(model.parameters(), opt_type='sgd', lr=0.1, momentum=0.5, nesterov=True)
    lr_sch = LRSF.create(optimizer, lrsch_type='exp_lr', 
                         decay_rate=0.5, decay_steps=1000, hold_steps=1000, 
                         min_lr=1e-5, warmup_steps=10, update_lr_on_opt_step=True)
    metrics = { 'acc': CategoricalAccuracy() }
    
    trainer = PLDATrainer(
        model, optimizer, epochs=25,
        device='cpu', metrics=metrics, lr_scheduler=lr_sch,
        loss_weights={'multi': 0, 'bin': 1}, p_tar=0.05,
        exp_path=output_path + '/dplda')
                          
    trainer.fit(train_loader, val_loader)
    plda = SPLDA(mu=model.mu.detach().cpu().numpy(), 
...
def train_be(x_et, x_trn, class_ids_trn, u2c_trn,
             lda_dim, plda_type, y_dim, z_dim,
             epochs, ml_md, md_epochs, k_nn_1, k_nn_2, pca_var_r,
             w_mu, w_B, w_W, output_path):
    ...
    logging.info('Select num_spks={} num_segments={}'.format(np.max(class_ids)+1, x.shape[0]))
    ...
    elbo = plda.fit(x_ln, class_ids, epochs=epochs,
                    ml_md=ml_md, md_epochs=md_epochs)
    ...
    logging.info('Select num_spks={} num_segments={} {}'.format(np.max(class_ids)+1, x.shape[0], k_nn_2))
    # Adapt PLDA
    if pca:
        x = pca.predict(x)
    x_lda = lda.predict(x)
...
def train_bes(v_file_train, train_list,
              v_file_enroll_test, enroll_test_list,
              lda_dim, plda_type, y_dim, z_dim,
              epochs, ml_md, md_epochs, k_nn_1, k_nn_2,
              w_mu, w_B, w_W, pca_var_r,
              output_path, part_idx, num_parts, **kwargs):
    ...
    for c in range(num_classes_et):
        ...
        train_be(x_et, x_trn, class_ids_trn, u2c_trn,
                 lda_dim, plda_type, y_dim, z_dim,
                 epochs, ml_md, md_epochs, k_nn_1, k_nn_2, pca_var_r,
                 w_mu, w_B, w_W, output_path_c)
