...
score_dir=exp/scores/$nnet_name/cosine
...
if [ $stage -le 2 ]; then
    ...
    steps_be/eval_tel_be_cos.sh --cmd "$train_cmd"  \
...
score_plda_dir=$score_dir/cosine_snorm_${coh_data}${ncoh}

if [ $stage -le 4 ]; then

    echo "eval SRE16 with S-Norm"
    steps_be/eval_tel_be_cos_snorm.sh --cmd "$train_cmd"  --ncoh $ncoh \
				     data/sre16_eval40_yue_test/trials \
				     data/sre16_eval40_yue_enroll/utt2spk \
				     $xvector_dir/sre16_eval40_yue/xvector.scp \
