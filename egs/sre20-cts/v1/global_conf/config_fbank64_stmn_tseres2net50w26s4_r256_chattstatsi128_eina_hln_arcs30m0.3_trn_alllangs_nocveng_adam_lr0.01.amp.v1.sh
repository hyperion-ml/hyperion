...
batch_size_1gpu=5
eff_batch_size=510 # effective batch size
ipe=$nnet_num_augs
min_chunk=4
max_chunk=4
lr=0.01
...
nnet_opt="--resnet-type $nnet_type --in-feats 64 --in-channels 1 --in-kernel-size 3 --in-stride 1 --no-maxpool --norm-layer instance-norm-affine --head-norm-layer layer-norm  --no-maxpool --res2net-width-factor $width_factor --res2net-scale $scale --pool-type ch-wise-att-mean-stddev --pool-inner-feats $attstats_inner --se-r $se_r"
...
lrs_opt="--lrsched.lrsch-type exp_lr --lrsched.decay-rate 0.5 --lrsched.decay-steps 15000 --lrsched.hold-steps 60000 --lrsched.min-lr 1e-5 --lrsched.warmup-steps 2000 --lrsched.update-lr-on-opt-step"

nnet_name=${feat_type}_${nnet_type}${ws_tag}_r${se_r}_eina_hln_chattstatsi${attstats_inner}_e${embed_dim}_arcs${s}m${margin}_do${dropout}_adam_lr${lr}_b${eff_batch_size}_amp.v1.$nnet_data
nnet_num_epochs=100
