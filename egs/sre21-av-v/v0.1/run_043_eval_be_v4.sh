...
be_vid_name=be_v4_thrahc${thr_ahc}
...
if [ $stage -le 4 ]; then
  ...
  echo "SRE21"
  steps_be/eval_face_vid_be_v4.sh \
    --cmd "$train_cmd" --thr-ahc $thr_ahc \
...
if [ $stage -le 9 ]; then
  ...
  #SRE21
  echo "SRE21 S-Norm"
  steps_be/eval_face_vid_be_snorm_v4.sh \
    --cmd "$train_cmd" --ncoh $ncoh --thr-ahc $thr_ahc \
    data/sre21_visual_dev_test/trials \
    data/sre21_visual_dev_enroll/utt2spk \
    $face_embed_ref_dir/sre21_visual_dev_enroll/embed.scp \
    $face_embed_ref_dir/sre21_visual_dev_enroll/embed.scp \
...
if [ $stage -le 10 ]; then
  ...
  echo "JANUS S-Norm"
  steps_be/eval_face_vid_be_snorm_v4.sh \
    --cmd "$train_cmd" --ncoh $ncoh --ncoh-discard 100 --thr-ahc $thr_ahc \
    data/janus_dev_test_core/trials \
    data/janus_dev_enroll/utt2spk \
    $face_embed_ref_dir/janus_dev_enroll/embed.scp \
    $face_embed_facedet_dir/janus_dev_enroll/embed.scp \
