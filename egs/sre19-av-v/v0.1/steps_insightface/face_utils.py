...
def read_video(file_path, fps):
    ...
    video_fps = float(f.streams.video[0].average_rate.numerator)/f.streams.video[0].average_rate.denominator
    delta = video_fps/fps
    frames = []
    frame_idx = []
    next_frame = 0
...
def save_facecrop_images(key, idx, frame, faces, facecrop_dir):
    ...
    for i in range(faces.shape[0]):
        ...
        logging.info('file %s frame %d face %d saved to %s' % (key,idx, i, img_filename))
...
def extract_embed_in_frame_v6(extractor, frame, faces, landmarks=None, thresh=1, use_retina=False):
    ...
    for i in range(faces.shape[0]):
        ...
        if frame2_i is None and not use_retina:
            logging.info('could not extracting embedding from face-size=%s ' % (str(frame_i.shape[1:])))
            continue

        if frame2_i is None:
            if landmarks is None:
                logging.info('failed mtcnn alignment from face-size=%s, just resizing to (112,112) ' % (str(frame_i.shape[1:])))
                frame2_i = face_preprocess.preprocess(frame, bbox, None, image_size='112,112')
                frame2_i = cv2.cvtColor(frame2_i, cv2.COLOR_BGR2RGB)
                frame2_i = np.transpose(frame2_i, (2,0,1))
            else:
                logging.info('resizing using retina landmarks face-size=(%d,%d)' % (bbox[3]-bbox[1]+1, bbox[2]-bbox[0]+1))
                landmarks_i = landmarks[i]
                frame2_i = face_preprocess.preprocess(frame, bbox, landmarks_i, image_size='112,112')
                frame2_i = cv2.cvtColor(frame2_i, cv2.COLOR_BGR2RGB)
                frame2_i = np.transpose(frame2_i, (2,0,1))
...
def extract_embed_in_frame_v5(extractor, frame, faces, landmarks=None, thresh=1, use_retina=False):
...
def extract_embed_in_frame_v4(extractor, frame, faces, landmarks=None, thresh=1, use_retina=False, x_dim=512):
...
def compute_overlap(bbox, bbox_ref):
    ...
    logging.info('Overlap bbox=%s bbox-ref=%s r=%.3f d=%.2f' % (str(bbox), str(bbox_ref), r, d))
