...
class SCPList(object):
    ...
    def save(self, file_path, sep=' ', offset_sep=':'):
        ...
        with open(file_path, 'w') as f:
            if self.offset is None:
                ...
                for k, p, o, r in zip(self.key, self.file_path, self.offset, range_spec):
    ...
    def parse_script(script, offset_sep):
        ...
        """
        file_range = [f.split('[', 1) for f in script]
        offset = None
        range_spec = None
        
        file_offset = [f[0].split(offset_sep, 1) for f in file_range]
        ...
        if do_range:
            range_spec1 = [f[1].rstrip(']').split(':', 1)
                           if len(f)==2 else None for f in file_range]
            range_spec21 = [int(f[0]) if f is not None and f[0].isdecimal()
    ...
    def load(cls, file_path, sep=' ', offset_sep=':'):
        ...
        with open(file_path, 'r') as f:
            fields = [line.rstrip().split(sep, 1) for line in f]
    ...
    def __eq__(self, other):
        ...
        if (self.offset is None and other.offset is not None or
            self.offset is not None and other.offset is None):
            eq = False
        elif self.offset is not None and other.offset is not None:
            eq = eq and np.all(self.offset == other.offset)

        if (self.range_spec is None and other.range_spec is not None or
            self.range_spec is not None and other.range_spec is None):
            eq = False
