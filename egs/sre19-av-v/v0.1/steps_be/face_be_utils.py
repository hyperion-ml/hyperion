...
def fill_missing_ref_with_facedet_avg(x_ref, x_e, seg_names):
    ...
    for i in range(len(x_ref)):
        if x_ref[i].shape[0] == 0:
            if x_e[i].shape[0] > 0:
                logging.warning('Empty reference for enroll %s, we put the average of faces in enroll file' % (seg_names[i]))
                x_ref[i] = np.mean(x_e[i], axis=0, keepdims=True)
            else:
                logging.warning('Empty reference for enroll %s, we use zero vector, no faces were detected in enroll file ' % (seg_names[i]))
