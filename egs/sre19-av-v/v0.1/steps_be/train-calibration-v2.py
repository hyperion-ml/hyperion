...
def train_calibration(score_file, key_file, model_file, prior, lambda_reg, verbose):
    ...
    mu_tar = np.mean(tar)
    mu_non = np.mean(non)
    # std = np.std(np.concatenate((tar - mu_tar, non - mu_non)))
    std_tar = np.std(tar)
    std_non = np.std(non)
    n_extra = int(1e6)
    tar_extra = mu_tar + std_tar * np.random.randn(n_extra)
    non_extra = mu_non + std_non * np.random.randn(n_extra)
    ...
    x = np.concatenate((tar, tar_extra, non, non_extra))
    y = np.concatenate(
        (
