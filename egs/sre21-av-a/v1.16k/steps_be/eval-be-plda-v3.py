...
def eval_plda(
    ...
    **kwargs,
    ...
    enroll, enroll_idx, ids_e = np.unique(
        enroll, return_index=True, return_inverse=True
    )

    models = {}
    model_file = f"{model_file_basename}_cts.h5"
    logging.info("loading plda model cts: %s", model_file)
    models["cts_cts"] = F.load_plda(plda_type, model_file)
    model_file = f"{model_file_basename}_afv.h5"
    logging.info("loading plda model afv: %s", model_file)
    models["afv_afv"] = F.load_plda(plda_type, model_file)
    logging.info("making cross-source plda")
    models["cts_afv"] = models["cts_cts"].copy()
    models["cts_afv"].weighted_avg_model(models["afv_afv"], 0.5, 0.5, 0.5)

    # assume all enrollment-side segments are from same source!
    source_e_model = source_e[enroll_idx]

    t1 = time.time()
    logging.info("computing llr")
    scores = np.zeros((len(enroll), len(x_t)), dtype=float_cpu())
    score_mask = np.zeros((len(enroll), len(x_t)), dtype=bool)
    for se in ["cts", "afv"]:
        for st in ["cts", "afv"]:
            logging.info("eval %s vs %s", se, st)
            idx_e_model = source_e_model == se
            idx_e = source_e == se
            idx_t = source_t == st
            cur_cond = f"{se}_{st}"
            if cur_cond not in models:
                cur_cond = f"{st}_{se}"
            idx = np.ix_(idx_e_model, idx_t)
            if np.sum(idx_e) == 0 or np.sum(idx_t) == 0:
                continue

            _, ids_se = np.unique(ids_e[idx_e], return_inverse=True)
            scores[idx] = models[cur_cond].llr_Nvs1(
                x_e[idx_e],
                x_t[idx_t],
                method="vavg-lnorm",
                ids1=ids_se,
    ...
    assert np.all(score_mask), "not all scores were computed"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-file-basename", required=True, help=("model file"))
