...
def write_wav(segments, target_fs, wav_dir, output_file):
...
def make_enroll_dir(df_enr, wav_dir, target_fs, output_path):
    ...
    segments = df_enr["segment_id"].sort_values(ascending=True).unique()
    ...
    write_wav(segments, target_fs, wav_dir, enroll_dir / "wav.scp")
    ...
    df_segms = (
        df_enr[["segment_id", "source_type"]]
        .drop_duplicates()
        .sort_values(by="segment_id")
    )
    df_segms.to_csv(enroll_dir / "segments.csv", sep=",", index=False)
...
def make_test_dir(df_model, df_key, wav_dir, target_fs, output_path):
    ...
    write_wav(df_segms["segment_id"], target_fs, wav_dir, test_dir / "wav.scp")
...
def prepare_sre21av_eval_audio(
    ...
    key_file = corpus_dir / "docs" / "sre21_audio_eval_trials.tsv"
    ...
    df_model = make_enroll_dir(df_enr, wav_dir, target_fs, output_path)
    make_test_dir(df_model, df_key, wav_dir, target_fs, output_path)
    ...
    make_test_dir(df_model, df_key, wav_dir, target_fs, av_output_path)
