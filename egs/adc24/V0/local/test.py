...
def prepare_adi17(corpus_dir, output_dir, remove_langs, map_langs_to_lre_codes,target_fs, verbose):
    ...
    dev_files = glob.glob(str(corpus_dir / "dev_segments/*.wav"),
                          recursive=True)
    dev_ids = [Path(f).stem for f in dev_files]
    df_dev_labels = pd.read_csv(str(corpus_dir / "adi17_official_dev_label.txt"),
                    delim_whitespace=True)
    df_dev_labels = df_dev_labels.set_index("id")
    dev_langs = df_dev_labels.loc[dev_ids, "label"].values
    
    test_files = glob.glob(str(corpus_dir / "test_segments/*.wav"),
                           recursive=True)
    test_ids = [Path(f).stem for f in test_files]
    df_test_labels = pd.read_csv(str(corpus_dir / "adi17_official_test_label.txt"),
                    delim_whitespace=True)
    df_test_labels = df_test_labels.set_index("id")
    test_langs = df_test_labels.loc[test_ids, "label"].values
    ...
    #all_ids = train_ids + dev_test_ids (Gl3th)
    #all_files = train_files + dev_test_files (Gl3th)
    #all_langs = list(train_langs) + list(dev_test_langs) (Gl3th)
    if map_langs_to_lre_codes:
        all_langs = map_to_adc(all_langs)

    train_ids = ids_format(train_ids,train_langs)
    test_ids = ids_format(test_ids,test_langs)
    dev_ids = ids_format(dev_ids,dev_langs)
