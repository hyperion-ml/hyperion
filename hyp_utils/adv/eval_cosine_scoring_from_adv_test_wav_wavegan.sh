#!/bin/bash
# Copyright 2020 Johns Hopkins University (Jesus Villalba, Sonal Joshi)  
# Apache 2.0.
#
nj=20
cmd=run.pl
...
if [ $# -ne 7 ]; then
  ...
  echo "  --smoothing-after-wavegan <bool>                 # if true smoothing is done after wavegan"
  echo "  --wave-gan-root-dir <str>                        # path to wavegan root directory"
  echo "  --wave-gan-model-ckpt <str>                      # model checkpoint for wavegan"
  exit 1;
fi
