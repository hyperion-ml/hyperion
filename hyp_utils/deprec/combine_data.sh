...
#           2014  David Snyder
#           2019  Jesus Villalba modifies to solve problem mergin voxceleb and sitw
...
for file in utt2spk utt2lang utt2dur reco2dur feats.scp text cmvn.scp vad.scp reco2file_and_channel wav.scp spk2gender $extra_files; do
  ...
  if ! $absent_somewhere; then
    set -o pipefail
    ( for f in $*; do cat $f/$file; done ) | sort -k1,1 > $dest/$file || exit 1;
    set +o pipefail
...
if ! $skip_fix ; then
  hyp_utils/fix_data_dir.sh $dest || exit 1;
fi
